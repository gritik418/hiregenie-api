...
4. STRICT SKILL & KEYWORD MATCHING RULES:
   - If a technology/tool required by the job description or expected for the target Job Title is NOT explicitly written in the candidate's resume, it is STRICTLY FORBIDDEN to classify it as matched. It MUST be classified as "missing".
   - You must never assume the candidate knows a technology because they have a related skill. E.g.:
     * Do NOT assume they know ToolA because they know ToolB.
     * Do NOT assume they know ToolC because they know ToolD.
     * Do NOT assume they have experience in Domain A because they have experience in Domain B.
5. STRICT RESPONSIBILITY MATCHING:
   ...
   - If the job description requires a responsibility (e.g., specific tasks and duties requested in the job description) and the candidate's resume does not show concrete evidence of performing this duty, it MUST be classified as "missing" under "responsibilities.missing".
...
3. "roleFit.alignmentScore" represents how well the candidate's background/history/domain aligns with the target Job Title:
   - If the candidate's background is in a completely different domain (e.g., Domain A candidate applying for a Domain B role), the alignmentScore MUST be between 0 and 20.
